...
interface CodeEditorProps {
  ...
  onConfirmPlayer: (playerId: 1 | 2) => void;
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          <button
            type="button"
            onClick={onValidate}
            className="flex-1 min-w-[140px] py-2.5 rounded-lg border border-cyan-700/50 bg-cyan-950/30 hover:bg-cyan-900/30 text-cyan-300 font-mono text-xs font-bold transition-all"
          >
            VALIDATE CODE
          </button>
          <button
            type="button"
            onClick={onConfirm}
            disabled={!player.validation?.isValid || !player.name.trim()}
            className="flex-1 min-w-[140px] py-2.5 rounded-lg border border-emerald-700/50 bg-emerald-950/30 hover:bg-emerald-900/30 text-emerald-300 font-mono text-xs font-bold transition-all disabled:opacity-40 disabled:cursor-not-allowed"
          >
            CONFIRM PLAYER
          </button>
        </div>
        ...
        >
          {player.validation?.isValid && (
            <span className="flex items-center gap-1.5">
              ...
              Strategy validated. This player can now be confirmed.
            </span>
          )}
    ...
    <div className="space-y-6 max-w-7xl mx-auto px-4 py-4 font-sans text-slate-100">
      <div className="text-center space-y-2 py-4">
        ...
        <p className="text-xs md:text-sm text-slate-400 max-w-2xl mx-auto">
          Validate two restricted Python-like strategies, confirm each player, then lock the match and run the full 100 x 200 simulation.
        </p>
      </div>
            ...
            <div className="space-y-2 text-[11px] font-mono">
              {setupIssues.length === 0 ? (
                <div className="rounded-lg border border-emerald-900/40 bg-emerald-950/20 px-3 py-2 text-emerald-300">
                  Both players are validated, confirmed, and ready to lock the match.
                </div>
              ) : (
