...
import { isValidActionCode } from './actionCodes';
import type {
  BinaryExpression,
  Expression,
  IfStatement,
...
export function validateStrategy(code: string): ValidationResult {
  ...
  try {
    ...
    if (nextIndex !== lines.length) {
      ...
      throw new Error(`Unexpected statement on line ${line.lineNumber}.`);
    }
...
export function executeStrategy(
  ...
  try {
    for (const statement of program.body) {
      const resolved = executeStatement(statement, cell, environment);
      if (resolved) {
...
function parseReturnStatement(text: string, lineNumber: number) {
...
function parseExpression(source: string, lineNumber: number): Expression {
  ...
  function parsePrimary(): Expression {
    ...
    if (token.type === 'identifier') {
      ...
      const isAllowedKey =
        sourceName === 'cell'
          ? ALLOWED_CELL_KEYS.has(keyToken.value)
          : ALLOWED_ENVIRONMENT_KEYS.has(keyToken.value);
      if (!isAllowedKey) {
...
function executeStatement(
  ...
  if (evaluateExpression(statement.condition, cell, environment)) {
